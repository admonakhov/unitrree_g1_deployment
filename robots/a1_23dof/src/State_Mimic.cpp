...
: FSMState(state_mode, state_string) 
{
    ...
    if (param::config["Simulation"]) {
        if (param::config["Simulation"]["enabled"]) {
            sim_enabled = param::config["Simulation"]["enabled"].as<bool>();
        }
        if (param::config["Simulation"]["play_audio"]) {
            play_audio = param::config["Simulation"]["play_audio"].as<bool>();
        }
    }
    audio_enabled_ = play_audio && !sim_enabled;
    if (!audio_enabled_) {
        spdlog::info("Audio disabled (Simulation.enabled={}, Simulation.play_audio={})",
                     sim_enabled, play_audio);
    }
...
{
    if (!audio_enabled_) {
        spdlog::info("Audio disabled, skipping AudioClient init");
        return;
    }
    if (!audio_client_) {
